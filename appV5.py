...
class _ProcessTableRecorder:
    def __init__(
        ...
        *,
        ...
        self.had_rows = False
        self.rows: list[_ProcessRowEntry] = []
        self._index: dict[str, int] = {}

    def _lookup_meta(self, key: str | None) -> Mapping[str, Any] | None:
        return _lookup_process_meta_entry(self.process_meta, key)
    ...
    def add_row(
        ...
        self.add_labor_cost_line(display_label, cost_val)
...
def _merge_process_meta_entries(
...
def _fold_process_meta_entries(
    ...
    for raw_key, raw_meta in meta_source.items():
        ...
        folded[canon_key] = _merge_process_meta_entries(existing, raw_meta)
...
def _merge_applied_process_details(entries: Iterable[Mapping[str, Any]]) -> dict[str, Any]:
    merged: dict[str, Any] = {}
    notes: list[str] = []
    seen_notes: set[str] = set()
...
def _fold_applied_process_entries(
    ...
    for canon_key, entries in grouped.items():
        merged_entry = _merge_applied_process_details(entries)
...
def _lookup_process_meta_entry(
...
def render_quote(  # type: ignore[reportGeneralTypeIssues]
    ...
    def add_process_notes(key: str, indent: str = "    "):
        ...
        meta = _lookup_process_meta_entry(process_meta, key) or {}
        hr_val = stored_hours
        if hr_val <= 0:
            try:
    ...
    process_meta, bucket_alias_map = _fold_process_meta_entries(process_meta_raw)
    applied_process = _fold_applied_process_entries(applied_process_raw, bucket_alias_map)
    ...
    for raw_key, raw_amount in legacy_cost_items:
        ...
        meta_entry = _lookup_process_meta_entry(process_meta, canon_key)
        hours_val = 0.0
        minutes_val = 0.0
    ...
    process_table = _ProcessTableRecorder(
        cfg=cfg,
    ...
    rows: tuple[_ProcessRowEntry, ...] = tuple(getattr(process_table, "rows", ()))

    def _process_row_canon(record: _ProcessRowEntry) -> str:
        if record.canon_key:
            return record.canon_key
        return _canonical_bucket_key(record.name)

    def _find_process_row(target_canon: str) -> _ProcessRowEntry | None:
        if not target_canon:
            return None
        for record in rows:
