...
def render_quote(
    ...
    def _add_labor_cost_line(
        ...
        *,
        ...
        canonical_label, _ = _canonical_amortized_label(display_label)
        detail_key = canonical_label or display_label
        labor_costs_display[detail_key] = labor_costs_display.get(detail_key, 0.0) + amount_val
        existing_detail = labor_cost_details.get(detail_key)
        merged_detail = _merge_detail(existing_detail, detail_bits or [])
        detail_to_write: str | None
        if merged_detail:
            labor_cost_details[detail_key] = merged_detail
            detail_to_write = merged_detail
        elif fallback_detail:
            detail_to_write = fallback_detail
            labor_cost_details.setdefault(detail_key, fallback_detail)
        else:
