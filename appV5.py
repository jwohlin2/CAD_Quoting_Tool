...
def render_quote(  # type: ignore[reportGeneralTypeIssues]
    ...
    if not labor_costs_display:
        for raw_key, amount in (process_costs or {}).items():
            norm = _normalize_bucket_key(raw_key)
            if not norm or norm.startswith("planner_"):
                continue
            try:
                numeric_amount = float(amount or 0.0)
            except Exception:
                continue
            label = _process_label(raw_key)
            labor_costs_display[label] = labor_costs_display.get(label, 0.0) + numeric_amount

    if not labor_costs_display:
        for label, amount in labor_cost_totals.items():
            try:
                labor_costs_display[label] = float(amount or 0.0)
            except Exception:
                continue
    else:
        for label, amount in labor_cost_totals.items():
    ...
    if not bucket_table_rows:
        for label, amount in labor_costs_display.items():
            try:
                numeric_amount = float(amount or 0.0)
            except Exception:
                numeric_amount = 0.0
            if not ((numeric_amount > 0) or show_zeros):
                continue
            ...
            if detail_text not in (None, ""):
                ...
                if canon_key:
                    add_process_notes(canon_key, indent="    ")
            display_labor_for_ladder += numeric_amount
    ...
    if not roughly_equal(ladder_subtotal, printed_subtotal, eps=0.01):
        ladder_subtotal = printed_subtotal
        ladder_labor = max(ladder_subtotal - ladder_directs, 0.0)
        ladder_base_labor = max(
            ladder_labor - (amortized_nre_total if qty > 1 else 0.0),
            0.0,
        )
        display_machine = max(
            computed_total_labor_cost - display_labor_for_ladder - pass_through_labor_total,
            0.0,
        )
        computed_subtotal = ladder_subtotal
