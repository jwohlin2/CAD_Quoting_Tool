...
def render_quote(  # type: ignore[reportGeneralTypeIssues]
    ...
    try:
        ...
        def _ensure_list(raw: Any, fallback: Any = None) -> list[Any]:
            source = raw
            ...
            if isinstance(source, _MappingABC):
                return [v for v in source.values()]
            if isinstance(source, str):
                stripped = source.strip()
                if not stripped:
                    return []
                return [stripped]
            if source is fallback and isinstance(fallback, (list, tuple, set)):
        ...
        subtotal_min, seen_deep, seen_std = _render_time_per_hole(
            time_lines, bins=bins, index_min=index_min, peck_min_deep=peck_min_deep, peck_min_std=peck_min_std,
        )
