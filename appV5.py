...
def render_quote(
    ...
    for entry in _iter_ordered_process_entries(
        ...
        display_label = entry.display_override or entry.label
        try:
            amount_for_display = float(entry.amount or 0.0)
        except Exception:
            amount_for_display = 0.0

        if display_label.strip().lower() == "misc":
            if planner_bucket_display_map or amount_for_display < 1.0:
                continue

        canonical_label, is_amortized = _canonical_amortized_label(entry.label)
        if (
            is_amortized
            and canonical_label
            in {"Programming (amortized)", "Fixture Build (amortized)"}
