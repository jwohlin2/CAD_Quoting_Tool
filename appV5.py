...
def _canonicalize_pass_through_map(
    data: Mapping[str, Any] | None,
) -> dict[str, float]:
    """Return a new dict with canonical pass-through labels."""
    ...
    for raw_label, raw_value in data.items():
        label = _canonical_pass_label(raw_label)
        if not label:
            continue
...
def render_quote(
    ...
    pass_through = _canonicalize_pass_through_map(pass_through_raw)
