...
def compact_dict(
    ...
    *,
    drop_values: Iterable[Any] = (None, "", [], {}),
) -> dict[str, Any]:
    """Return a shallow copy of *data* without empty-like values."""

    cleaned: dict[str, Any] = {}
    drop = tuple(drop_values)
    for key, value in data.items():
        if any(value == sentinel for sentinel in drop):
            continue
        cleaned[key] = value
    return cleaned
...
def compute_quote_from_df(df: pd.DataFrame,
    ...
    if "material_group" not in material_selection:
        fallback_group = _material_group_from_name(canonical_material or material_name)
        if fallback_group:
            material_selection["material_group"] = str(fallback_group).strip()
