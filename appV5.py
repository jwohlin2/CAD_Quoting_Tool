...
def render_quote(  # type: ignore[reportGeneralTypeIssues]
    ...
    def _compute_scrap_mass_g(
        *,
        removal_mass_g_est: Any,
        scrap_pct_raw: Any,
        effective_mass_g: float | None,
        net_mass_g: float | None,
    ) -> float | None:
        removal_est_val = _coerce_float_or_none(removal_mass_g_est)
        if removal_est_val is not None and removal_est_val > 0:
            return float(removal_est_val)

        scrap_fraction = normalize_scrap_pct(scrap_pct_raw)
        if scrap_fraction is not None:
            base_mass: float | None = None
            if effective_mass_g is not None and effective_mass_g > 0:
                base_mass = float(effective_mass_g)
            elif net_mass_g is not None and net_mass_g > 0:
                base_mass = float(net_mass_g)
            if base_mass is not None:
                return max(0.0, float(scrap_fraction) * base_mass)

        if effective_mass_g is not None and net_mass_g is not None:
            return max(0.0, float(effective_mass_g) - float(net_mass_g))
