...
def render_quote(
    ...
    prepared_bucket_view = (
        _prepare_bucket_view(bucket_view) if isinstance(bucket_view, _MappingABC) else None
    )
    if isinstance(prepared_bucket_view, _MappingABC):
        order = prepared_bucket_view.get("order")
        buckets = prepared_bucket_view.get("buckets")
