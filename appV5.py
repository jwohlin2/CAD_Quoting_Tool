...
def coerce_checkbox_state(value: Any, default: bool = False) -> bool:
    ...
    if text in truthy or text.startswith("y"):
        return True
    if text in falsy or text.startswith("n"):
        return False

    for part in re.split(r"[/|,\s]+", text):
        if part in truthy or part.startswith("y"):
            return True
        if part in falsy or part.startswith("n"):
            return False
...
from cad_quoter.app.quote_services import QuoteConfiguration, infer_geo_override_defaults
...
_LLM_DELEGATE: Any | None = None


def configure_llm_integration(delegate: Any) -> None:
    """Record the configured LLM integration for backwards compatibility."""

    global _LLM_DELEGATE
    _LLM_DELEGATE = delegate
