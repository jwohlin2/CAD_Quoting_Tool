...
_DIA_VALUE_PATTERN = r"(?:\d+\s*/\s*\d+|\d*\.\d+|\.\d+)"
_RE_DIA_ANY = re.compile(
    rf"(?:Ø|⌀|DIA|O)\s*({_DIA_VALUE_PATTERN})"
    rf"|\(({_DIA_VALUE_PATTERN})\s*Ø?\)"
    rf"|\b((?:\d*\.\d+)|(?:\.\d+))\b",
    re.I,
)
_RE_TAP = re.compile(
...
_RE_MM_IN_DIA = re.compile(rf"(?:Ø|⌀|O|DIA|\b)\s*({_DIA_VALUE_PATTERN})", re.I)
_RE_PAREN_DIA = re.compile(rf"\(({_DIA_VALUE_PATTERN})\s*Ø?\)")
_RE_DIA_SIMPLE = re.compile(rf"[Ø⌀\u00D8]?\s*({_DIA_VALUE_PATTERN})", re.I)
