...
def _convert_dwg_to_dxf(path: Path, *, target_version: str = "ACAD2018") -> tuple[Path, bool]:
    ...
    out_dir = Path(tempfile.mkdtemp(prefix="dwg2dxf_out_"))
    ...
    if exe_lower.endswith((".bat", ".cmd")):
        ...
        in_dir = Path(tempfile.mkdtemp(prefix="dwg2dxf_in_"))
        staged_path = in_dir / "source.dwg"
        try:
            shutil.copy2(path, staged_path)
        except OSError as exc:  # pragma: no cover - filesystem issues
            raise RuntimeError(f"Failed to stage DWG for conversion: {path}") from exc

        out_dxf = out_dir / (staged_path.stem + ".dxf")
        cmd = [
            str(converter),
            str(in_dir),
            str(out_dir),
