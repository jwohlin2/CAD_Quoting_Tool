...
MATERIAL_KEYWORDS: Dict[str, List[str]] = {
    ...
    "stainless": ["stainless", "stainless steel", "304 stainless", "316 stainless", "stainless scrap"],  # Removed "ss" - too ambiguous!
    ...
    "titanium": ["titanium", "titanium scrap"],  # Removed "ti" - too short
...
def _find_price_for_material(prices: Dict[str, float], material_key: str) -> Optional[Tuple[float, str]]:
    ...
    # Sort by name length to prefer shorter/generic names (e.g., "Stainless Steel" over "304 Stainless Steel")
    ...
    for material_name, price in prices.items():
        norm_name = _normalize_material_name(material_name)
        if len(norm_name) >= 5:  # Avoid short ambiguous matches like "steel"
            for keyword in keywords:
