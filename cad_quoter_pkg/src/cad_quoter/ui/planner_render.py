...
from cad_quoter.pricing.rate_defaults import (
    fallback_keys_for_mode as _fallback_keys_for_mode,
    fallback_rate_for_bucket as _fallback_rate_for_bucket,
    fallback_rate_for_role as _fallback_rate_for_role,
)
...
_MILLING_MACHINE_RATE = _fallback_rate_for_bucket("milling", roles=BUCKET_ROLE, mode="machine")
_MILLING_LABOR_RATE = _fallback_rate_for_bucket("milling", roles=BUCKET_ROLE, mode="labor")
...
def _lookup_bucket_rate(
    ...
    lookup_mode = "labor" if mode == "labor" else "machine"
    fallbacks = _fallback_keys_for_mode(lookup_mode)
    bucket_role = BUCKET_ROLE.get(search_key or "", BUCKET_ROLE.get("_default"))
    ...
    reference_key = canon_key or search_key
    return float(_fallback_rate_for_bucket(reference_key, roles=BUCKET_ROLE, mode=lookup_mode))
...
def _build_planner_bucket_render_state(
    ...
    *,
    ...
    def _rate_from(search_key: str, *, mode: str | None = None) -> float:
        if not search_key:
            return 0.0
        fallbacks = _fallback_keys_for_mode(mode)
        return float(
            _shared_lookup_rate(
                search_key,
                flat_rates,
    ...
    for canon_key in order:
        ...
        machine_rate_lookup = _rate_from(search_key, mode="machine")
        labor_rate_lookup = _rate_from(search_key, mode="labor")
...
def _display_rate_for_row(
    ...
    *,
    ...
    if rate_lookup <= 0.0:
        canon = _canonical_bucket_key(label) or _normalize_bucket_key(label)
        if canon:
            mode = _bucket_cost_mode(canon)
            lookup_mode = "labor" if mode == "labor" else "machine"
            rate_lookup = _fallback_rate_for_bucket(canon, roles=BUCKET_ROLE, mode=lookup_mode)
    ...
    *,
    ...
    def _bucket_rate(name: str, mode: str) -> float:
        ...
        for candidate in candidates:
            ...
            if rate:
                return float(rate)
        return float(_fallback_rate_for_role(role, mode=lookup_mode))
