...
def load_catalog(csv_path: str) -> Dict[str, Dict[float, List[StockItem]]]:
    with open(csv_path, newline="", encoding="utf-8-sig") as f:
        ...
        if missing:
            raise ValueError(
                f"CSV missing required headers: {missing}. Found: {list(hdrs.keys())}"
            )
...
def _get_catalog() -> Dict[str, Dict[float, List[StockItem]]]:
    ...
    if _CATALOG_CACHE is None:
        env = _get_env()
        csv_path = env.get("CATALOG_CSV_PATH") or os.getenv(
            "CATALOG_CSV_PATH", r"D:\\CAD_Quoting_Tool\\catalog.csv"
        )
...
def lookup_sku_and_price_for_mm(
    ...
    L_in, W_in, T_in = map(_mm_to_inches, (L_mm, W_mm, T_mm))
